...
export function buildEmisionesData(conteo: ConteoComprobantes) {
  return {
...
export function buildVentasData(
  data: VentaGenerada[],
  fechaFiltro: Date = new Date()
) {
  const fecha = fechaFiltro || new Date();
  ...
  const ventasPorDia = dias.map((dia) =>
    filtrada
      ...
      .reduce(
        (acc: number, cur: VentaGenerada) => acc + (cur.monto?.total || 0),
        0
      )
  );
  ...
  return {
    ...
    totals: {
      ventas: ventasPorDia.reduce((a: number, b: number) => a + b, 0),
      proformas: proformasPorDia.reduce((a: number, b: number) => a + b, 0),
    },
  };
}
