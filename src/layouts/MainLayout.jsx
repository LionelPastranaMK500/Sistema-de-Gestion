...
export default function MainLayout() {
    ...
    // ------- Overlay local controlado por eventos -------
    const [overlayVisible, setOverlayVisible] = useState(false);
    const [overlayOpacity, setOverlayOpacity] = useState(0); // 0..1
    const holdTimerRef = useRef(null);
    const fadeTimerRef = useRef(null);
    const startedAtRef = useRef(0);
    const configRef = useRef({ total: 7000, fade: 2000 });

    useEffect(() => {
        const onShow = (e) => {
            const total = e.detail?.total ?? 7000;
            const fade = e.detail?.fade ?? 2000;
            configRef.current = { total, fade };
            const hold = Math.max(0, total - 2 * fade);

            // Limpiar timers previos
            clearTimeout(holdTimerRef.current);
            clearTimeout(fadeTimerRef.current);

            // Mostrar inmediatamente y hacer fade-in (vía transición CSS)
            setOverlayVisible(true);
            // pequeño tick para que el navegador aplique la transición
            requestAnimationFrame(() => setOverlayOpacity(1));

            startedAtRef.current = Date.now();

            // Cuando termine el hold, lanzamos el fade-out
            holdTimerRef.current = setTimeout(() => {
                setOverlayOpacity(0); // empieza el fade-out
                fadeTimerRef.current = setTimeout(() => {
                    setOverlayVisible(false); // desmonta overlay al terminar fade
                }, fade);
            }, hold);
        };

        const onHide = () => {
            // Respetar mínimo total: si llegan a pedir hide antes del hold,
            // calculamos el tiempo restante para completar (hold + fade).
            const { total, fade } = configRef.current;
            ...
            if (remaining <= fade) {
                // Solo falta el fade: empezarlo ya
                setOverlayOpacity(0);
                fadeTimerRef.current = setTimeout(() => setOverlayVisible(false), Math.max(0, remaining));
            } else {
                // Aún queda hold: esperar (remaining - fade), luego hacer fade
                holdTimerRef.current = setTimeout(() => {
                    setOverlayOpacity(0);
                    fadeTimerRef.current = setTimeout(() => setOverlayVisible(false), fade);
                }, remaining - fade);
            }
        };
    ...
    return (
        <div className="flex min-h-screen">
            ...
            {/* IMPORTANTÍSIMO: relative para que el overlay sea LOCAL */}
            <main className="relative flex-1 bg-gray-50 p-6 min-w-0 overflow-auto">
                ...
                {/* Overlay local SOLO cuando overlayVisible === true */}
                {overlayVisible && (
                    <div
                        className="private-loader-overlay transition-opacity duration-[2000ms] ease-in-out"
                        style={{ opacity: overlayOpacity }}
                    >
                        <div className="private-loader">
                            {/* rueda + carretera */}
                            <svg viewBox="0 0 254.532 254.532" xmlns="http://www.w3.org/2000/svg" className="wheel">
                                <g>
                                    <path
                                        id="tire"
