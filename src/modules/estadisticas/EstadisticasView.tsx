...
const EstadisticasView = () => {
  const [data, setData] = useState<VentaGenerada[]>([]);

  const [fechaEmisiones, setFechaEmisiones] = useState<Date>(new Date());
  const [fechaVentas, setFechaVentas] = useState<Date>(new Date());

  const [totalVentas, setTotalVentas] = useState(0);
  const [totalProformas, setTotalProformas] = useState(0);

  const [emisiones, setEmisiones] = useState<ConteoComprobantes>({
    total: 0,
    facturas: 0,
    boletas: 0,
    notasCredito: 0,
  ...
  const [dataVentasFiltrada, setDataVentasFiltrada] = useState<VentaGenerada[]>(
    []
  );
  ...
  const fmtMesAnio = (d: Date | null): string => {
  ...
  useEffect(() => {
    ...
    if (fechaEmisiones && data.length > 0) {
      toast.success("Emisiones actualizadas", { autoClose: 1000 });
    }
  }, [fechaEmisiones, data]);
  ...
  useEffect(() => {
    ...
    const ventasDelMes = datosDelMes.filter((d) =>
      ["FACTURA ELECTRÓNICA", "BOLETA DE VENTA ELECTRÓNICA"].includes(
        d.tDocumento
      )
    );
    const proformasDelMes = datosDelMes.filter(
      (d) => d.tDocumento === "PROFORMA ELECTRÓNICA"
    );

    const totalVentasCalculado = ventasDelMes.reduce(
      (acc, cur) => acc + (cur.monto?.total || 0),
      0
    );
    const totalProformasCalculado = proformasDelMes.reduce(
      (acc, cur) => acc + (cur.monto?.total || 0),
      0
    );
  ...
  return (
    <div className="flex flex-col w-full h-full p-6">
      <div className="flex-shrink-0 flex justify-between items-center pb-4 border-b">
        <h2 className="font-bold text-gray-800 text-3xl ml-16">Estadísticas</h2>
      </div>
          ...
          <div className="flex justify-between items-center px-5 py-4 border-b">
            ...
            <div className="relative">
              <Calendar
                ...
                onChange={(e) => e.value && setFechaEmisiones(e.value)}
                dateFormat="mm/yy"
                view="month"
                showIcon={false}
            ...
            <div className="col-span-12 lg:col-span-6">
              ...
              <ul className="space-y-2">
                {emisionesData.labels.map((label, i) => {
                  const key = keyMap[label] as keyof ConteoComprobantes;
                  return (
                    <li
                      key={label}
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <span
                          ...
                          style={{
                            backgroundColor:
                              emisionesData.datasets[0].backgroundColor[i],
                          }}
                        />
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="relative">
                <Calendar
                  ref={refCalVen}
                  value={fechaVentas}
                  onChange={(e) => e.value && setFechaVentas(e.value)}
                  dateFormat="mm/yy"
                  view="month"
                  showIcon={false}
                  appendTo={document.body}
