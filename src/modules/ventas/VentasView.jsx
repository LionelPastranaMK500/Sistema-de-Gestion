...
import { PermIdentityTwoToneIcon, CloseIcon, MenuIcon } from "@constants/icons";
...
const VentasView = () => {
    ...
    const otrosModalItems = componentsVentas
        ...
        .map(({ name, action }) => ({
            label: `${name} ${(() => {
                switch (action) {
                    case 'guia': return guiasRemision && guiasRemision.length > 0 ? ' ✓' : '';
                    case 'adicionales': return datosAdicionales && datosAdicionales.length > 0 ? ' ✓' : '';
                    case 'condicionPago': return condicionPago && condicionPago.condicion ? ' ✓' : '';
                    default: return '';
                }
            })()}`,
