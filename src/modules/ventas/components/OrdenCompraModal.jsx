...
const OrdenCompraModal = ({ visible, onHide }) => {
    ...
    const [inputValue, setInputValue] = useState(ordenCompra || "");

    useEffect(() => {
        if (visible) {
            setInputValue(ordenCompra || "");
        }
    }, [visible, ordenCompra]);

    const handleSave = () => {
        setOrdenCompra(inputValue);
        onHide();
    };

    const headerContent = (
        <h2 className="text-xl font-bold">Agregar Orden de Compra</h2>
    );
