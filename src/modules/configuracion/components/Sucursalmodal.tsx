...
const SucursalModal = ({
  ...
  const { formData, handleChange, setFormData } = useFormInput<SucursalForm>({
    nombre: "",
    direccion: "",
  });
  ...
  const {
    items: almacenes,
    setItems: setAlmacenes,
    addItem: addAlmacen,
    removeItem: removeAlmacen,
    updateItem: updateAlmacen,
  } = useDynamicList<AlmacenItem>([]);
  ...
  useEffect(() => {
    if (visible) {
      if (mode === "edit" && sucursalData) {
        ...
        setAlmacenes(
          sucursalData.almacenes.map((nombre, i) => ({
            tempId: Date.now() + i,
            nombre,
          }))
        );
        ...
        setVendedores([]);
        setAlmacenes([{ tempId: Date.now(), nombre: defaultSucursal }]);
      }
    }
  }, [
    visible,
  ...
  const handleSubmit = () => {
    ...
    toast.success(
      `Sucursal ${action === "AGREGAR" ? "agregada" : "actualizada"} con éxito.`
    );
    onHide();
  };

  const getVendedorLabel = (email: string) => {
      ...
      <div className="space-y-6 p-6">
        ...
        <div>
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              type="button"
              onClick={() => addVendedor(selectedVendedor || null)}
          ...
          {vendedores.map((email) => (
            <div
              key={email}
              className="flex items-center justify-between bg-gray-50 px-3 py-2 mt-2 rounded-lg border border-gray-200"
            >
              <span className="text-sm text-gray-700">
                {getVendedorLabel(email)}
              </span>
              <button
                type="button"
