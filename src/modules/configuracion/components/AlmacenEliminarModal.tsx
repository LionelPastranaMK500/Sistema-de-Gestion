...
export default function AlmacenEliminarModal({
  ...
  onConfirm,
  almacenData,
}: AlmacenEliminarModalProps) {
  const headerContent = (
    <h3 className="text-xl font-bold">Confirmar Eliminación</h3>
  );

  const footerContent = (
    <div className="flex justify-end items-center gap-3 p-4 bg-white border-t border-gray-200 rounded-b-lg">
      <Button
        label="No"
        icon="pi pi-times"
        ...
        <div className="text-gray-700 text-sm leading-snug w-full">
          <div className="p-3 bg-red-50 border border-red-200 rounded-lg shadow-inner">
            <p className="mb-3 font-medium text-red-700">
              ¿Está seguro de que desea eliminar permanentemente el siguiente
              almacén?
            </p>
            ...
            <p className="mt-1 text-red-700 text-sm">
              Esta acción no se puede deshacer. Al eliminarlo, se perderá
              **permanentemente** toda la información, historial e inventario
              asociado a este almacén.
            </p>
          </div>
        </div>
      </div>
