...
import { getSucursales, Sucursal } from "@/services/generadorData";
...
export default function ConfigurarSucursal() {
  ...
  const [infoModalMode, setInfoModalMode] = useState<"add" | "edit">("add");
  ...
  const handleConfigurar = (sucursal: Sucursal) => {
        ...
        <div className="w-full max-w-4xl mx-auto bg-white shadow-md rounded-lg overflow-hidden">
          ...
          <div className="divide-y divide-gray-200">
            {sucursales.map((sucursal) => (
              ...
              >
                <div className="col-span-5 font-medium">{sucursal.nombre}</div>
                <div className="col-span-3 text-gray-500">
                  {sucursal.vendedores?.length > 0
                    ? `${sucursal.vendedores.length} Vendedores`
                    : "-"}
                </div>
